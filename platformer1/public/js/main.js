...
game.fullScreen = true // can be false , it is used to stretch to full screen

//preload assets
let bg = game.preload("bg", "maps/composed-bg.png")
let bg2 = game.preload("bg2", "maps/background.png")
...
//for debugging in dev mode not for production
let maxEntityText = new UseText(" MaxEntities   :  ", game.maxEntity, 17, 150, 10, "lavender")

let bgSound = new PlaySound(bgMusic, 0.1, "loop") // load background music
...
export let msgBox = document.createElement("div")
msgBox.classList.add("msg")
msgBox.innerHTML = `
<h2>Hello Gamers , congratulations !!</h2>
<p>You have successfully completed the game .</br></br></br>
...
play.onclick = function () {
    ...
    maxEntityText.visible = true
    bgSound.run()
    uiContainer.style.display = 'none'
    play.style.display = "none"
    if (screen.width <= 769) {
        jumpB.visible("visible")
        controller.visible("visible", [0, 1])
    }
}
...
game.complexUpdate = () => {
    ...
    maxEntityText.update(game.maxEntity)
}
